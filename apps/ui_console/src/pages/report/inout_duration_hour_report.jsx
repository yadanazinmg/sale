...
import { dateFormatter, dateFormatterReport } from "../../helpers/ag_grid_helpers";
import { HourExcelExport } from "../../helpers/excel_helper";

// const dateFormatter = (params) => {
//   return moment(params.value).format("DD/MM/YYYY");
// };

const DurationHourReportPage = (props) => {
  ...
  const columnDefs = useMemo(
    () => [
      { field: "date", valueFormatter: dateFormatterReport },
      { field: "hour" },
      { field: "count" },
      { field: "amount" },
      // { field: "shift_id" },
  ...
  const handleExport = () => {
    HourExcelExport(parkingrecord, sdate, edate);
  };
  ...
  useEffect(async () => {
    ...
    if (!gqlLoading && ticketrecord) {
      ...
      if (tmp) {
        let i = 0;
        const reducer = (map, d) => {
          let user = new Date(d.exit_time).getHours();
          console.log(user);
          if (map[user]) {
            let a = map[user];
            a.date = d.exit_time;
  ...
  return (
    <div className="relative flex flex-col h-full">
      <div className="w-full flex flex-row py-1 place-content-center">
        <span className="text-3xl font-semibold capitalize mb-2">Hour Report</span>
      </div>
...
export default withUser(DurationHourReportPage);
