...
import { delete_redemption, get_redemptions } from "../../graphql/redemption";
...
const RedemptionPage = (props) => {
  ...
  const [deleteRedemption] = useMutation(delete_redemption);
  ...
  const handleCreate = () => {
    console.log("create");
    navigate(paths.redemption_create);
  };

  const handleEdit = (id) => {
    console.log("edit", id);
    navigate(paths.getRedemptionEdit(id));
  };
  ...
  const columnDefs = useMemo(
    () => [
      { headerName: "Vehicle Id", field: "vehicle_id" },
      // { headerName: "From Hour", field: "from_hour" },
      // { headerName: "From Min", field: "from_min" },
      // { headerName: "To Hour", field: "to_hour" },
      { headerName: "Discount Minute", field: "to_min" },
      { headerName: "Redemption Type", field: "redemption_type.name" },
  ...
  return (
    <div className="relative flex flex-col h-full">
      <div className="w-full flex flex-row px-6 py-0 lg:hidden place-content-center">
        <span className="text-3xl font-semibold capitalize ">Redemption</span>
      </div>
      <div className="w-full grid grid-cols-3 px-4 py-2 justify-between">
        ...
        <div className="px-6 py-2 hidden lg:flex place-content-center">
          <span className="pt-2 px-2 text-3xl font-semibold capitalize ">Redemption</span>
        </div>
        <div className="">&nbsp;</div>
      </div>
...
export default withUser(RedemptionPage);
