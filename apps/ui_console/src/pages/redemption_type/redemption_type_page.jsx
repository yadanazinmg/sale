...
import { delete_redemption_type, get_redemption_types } from "../../graphql/redemption_type";

const RedemptionTypePage = (props) => {
  const {
    loading: gqlLoading,
    error,
    data: gates,
  ...
  const [deleteRedemptionType] = useMutation(delete_redemption_type);
  ...
  const handleCreate = () => {
    console.log("create");
    navigate(paths.redemption_type_create);
  };

  const handleEdit = (id) => {
    console.log("edit", id);
    navigate(paths.getRedemptionTypeEdit(id));
  };
  ...
  const columnDefs = useMemo(
    () => [
      ...
      { headerName: "Discount Minute", field: "discount_minute" },
  ...
  return (
    <div className="relative flex flex-col h-full">
      <div className="w-full flex flex-row px-6 py-0 lg:hidden place-content-center">
        <span className="text-3xl font-semibold capitalize ">Redemption Type</span>
      </div>
      <div className="w-full grid grid-cols-3 px-4 py-2 justify-between">
        ...
        <div className="px-6 py-2 hidden lg:flex place-content-center">
          <span className="pt-2 px-2 text-3xl font-semibold capitalize ">Redemption Type</span>
        </div>
        <div className="">&nbsp;</div>
      </div>
...
export default withUser(RedemptionTypePage);
